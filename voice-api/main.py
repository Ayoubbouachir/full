...
try:
    ...
    print(f"Warning: Whisper import failed: {e}")
...
if WHISPER_AVAILABLE:
    try:
        ...
        try:
            ...
            print("[OK] Whisper (tiny) loaded!")
        except:
            print("[CRITICAL] Whisper failed completely.")
            WHISPER_AVAILABLE = False
else:
    print("Whisper not available, will use Gemini fallback.")
...
async def speech_to_text(file: UploadFile = File(...), field: str = Form("text")):
    ...
    try:
        ...
        if WHISPER_AVAILABLE and model:
            ...
            # FALLBACK TO GEMINI
            print("🚀 Whisper unavailable, falling back to Gemini for speech-to-text")
            return transcribe_audio_with_gemini(output_filename, context="general")
    finally:
...
async def product_voice_search(file: UploadFile = File(...)):
    ...
    try:
        ...
        if WHISPER_AVAILABLE and model:
            ...
            # FALLBACK TO GEMINI
            print("🚀 Whisper unavailable, falling back to Gemini for product search")
            gemini_res = transcribe_audio_with_gemini(tmp_output, context="product_search")
            if gemini_res.get("success"):
                # Gemini returns JSON if context="product_search"
                try:
                    res_data = json.loads(gemini_res["text"])
                    intent = res_data.get("intent", "search")
                    product_name = res_data.get("product", "")
                ...
                return {"success": False, "error": "Gemini fallback failed", "raw_text": ""}
        
        if WHISPER_AVAILABLE:
            logging.info(f"🔍 Direct Whisper text: {raw_text}")
            # Détection d'intention (Whisper only, Gemini already did it)
            intent = "search"
            product_name = raw_text
            ...
            logging.info(f"🔍 Gemini result for product search: {product_name}")
